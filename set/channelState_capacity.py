...
class ChannelState():
    ...
    def get_delay_from_payment_history(self, amount, current_time, Max_delay):
        ...
        for i in range(len(payment_history)) :
            ...
            # print('payment_history_i', payment_history[i])
            # print('payment_history_i_1', payment_history[i][1])
            temp_delay = current_time - payment_history[i][1]
            # print("temp_delay", temp_delay, i)

            # if  temp_delay >= Max_delay :
            #     print("current time", current_time)
            #     for j in range(i, len(payment_history)) :
                    # print("payment_history", payment_history, i)
                    # self.pop_payment_history(i)
                    # print("pop history")
                # break
            temp_add += payment_history[i][0]
            # print("previous temp_result", self.get_average_capacity())
