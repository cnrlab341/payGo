from message import fail, initContractPropose, contractPropose, contractSelect, LockedTransfer, sendSecret, resSecret, \
    revealSecret, resRevealSecret, completePayGo, waitContractSelect, selectionComplete, contractConfirm, \
    contractReject, endcontractPropose
...
from settingParameter import alpha, contract_meaningful_incentive_constant, contract_meaningful_delay_constant, \
    time_meaningful_constant, theta, min_probability
...
class Node:
    ...
    def init_contract_propose(self, round, initiator, target, amount, secret, start_time, Omega, Omega_prime,
                              payment_state, start):
        ...
        for recipient in initiator.partner.keys():
            if self.partner[recipient].check_balance(amount, self.RTT.get_accumulate_time()):
                success += 1
                ...
                if not cr in self.contract_table.keys():
                    ...
                    self.experiment_result.protocol_time[round] = {"propose": start_time, "select": 0,
                                                                   "lockTransfer": 0}
            ...
            if self.address == message.target.address:
                ...
                for partner in self.partner.keys():
                    possibility = self.contract_select(message.cr, partner, message.amount, message.producer,
                                                       self.contract_table[message.cr].receive_contract_bundle, False,
                                                       "propose")
                    if partner.line != message.producer.line and possibility != -1 and \
                        ...
                        if partner == message.target:
                            ...
                            create_bundle = contract_bundle(self.address).execute(channel_probability, Omega,
                                                                                  Omega_prime)
    ...
    def contract_select(self, cr, selected_consumer, amount, producer, contract_bundle, already_section, type):
        ...
        if already_section:
            ...
            if self.partner[selected_consumer].check_balance(0, self.RTT.get_accumulate_time()):
        ...
        if self.partner[selected_consumer].check_balance(amount, self.RTT.get_accumulate_time()):
            index = len(contract_bundle[producer]["Delay"]) - 1
            ...
            for item in list(pending_payment_bundle):
                ...
                if self.partner[selected_consumer].check_balance(total_amount, self.RTT.get_accumulate_time()):
                    # print("BBB")
                    for i in range(len(contract_bundle[producer]["Delay"]) - 1, -1, -1):
                        # delay * 5 -> 수정 가능
                        if delay <= contract_bundle[producer]["Delay"][i] / contract_meaningful_delay_constant:
    ...
    def receive_contract_select(self, message, count, payment_state, Omega, r):
        ...
        if self == message.initiator:
            ...
            M.append(
                self.send_contract_confirm(message.cr, selected_consumer, selected_contract[0], selected_contract[1]))
        else:
            sucess = 0
            aready_reservation = False
            for recipient in self.contract_table[message.cr].receive_contract_bundle.keys():
                selected_index = self.contract_select(message.cr, selected_consumer, amount,
                                                      recipient, table.receive_contract_bundle, aready_reservation,
                                                      "selection")
                ...
                if selected_index == -1:
                    ...
                    if not aready_reservation:
                        ...
                        temp2 = {
                            self.name: self.partner[selected_consumer].test_get_balance(self.RTT.get_accumulate_time())}
                        temp3 = {self.name: table.receive_contract_bundle[recipient]["Delay"][
                                                selected_index] / contract_meaningful_delay_constant}
                        ...
                        temp = {self.name: self.partner[selected_consumer].get_wait_confirm_count() +
                                           self.partner[selected_consumer].get_reserve_payment_count()}
    ...
    def receive_contract_reject(self, message, round):
        ...
        if message.type == "contractRejectDown":
            ...
            if self != table.initiator:
                ...
                try:
                    self.partner[self.contract_table[message.cr].producer].fail_payment(message.cr,
                                                                                        pending_payment[message.cr][0],
                                                                                        1)
                except:
                ...
                M.append(
                    self.send_contract_reject(message.cr, self.contract_table[message.cr].producer, "contractRejectUp"))
    ...
    def send_locked_transfer(self, cr, initiator, producer, recipient, target, amount, secretHash, receive_BP,
        ...
        new_time = time.time()
        if self.partner[recipient].check_balance2(cr, self.contract_table[cr].amount + result_incentive,
                                                  self.RTT.get_accumulate_time()):
            ...
            if self == initiator:
                ...
                if 20 < (int(new_time * time_meaningful_constant) - pending_payment[cr][
                    2]) / time_meaningful_constant + 0.008:
                    ...
                    initiator.experiment_result.onchain_access_selected_contract[r] = \
                        initiator.experiment_result.selected_contract[r][self.name], receive_aux_contract[1][
                            10] / contract_meaningful_delay_constant
                    ...
                    initiator.experiment_result.onchain_access_pendingQueue[r] = \
                    initiator.experiment_result.pending_queue[r][self.name]
    ...
    def receive_reveal_secret(self, message, round, Omega, Omega_prime, total_round):
        ...
        BP, finalAmount, final_incentive, final_delay, select_contract_index= self.partner[message.consumer].unlock(self.w3, message.cr,
                                                                                              message.secret,
                                                                                              message.endTime,
                                                                                              aux_incentive, aux_delay,
                                                                                              self, initiator)
        final_incentive = final_incentive / contract_meaningful_incentive_constant
        incentive = self.contract_table[message.cr].contract_bundle["Incentive"][select_contract_index] / contract_meaningful_incentive_constant
        ...
        if initiator != self:
            ...
            if self.line - initiator.line == 1 or self.line - initiator.line == -1:
                self.experiment_result.h1_contract_bundle.append(
                    self.contract_table[message.cr].receive_contract_bundle[producer])
                self.experiment_result.h2_contract_bundle.append(
                    self.contract_table[message.cr].contract_bundle[message.consumer])
                ...
                if round >= min and len(self.experiment_result.utility) <= result_count:
                    self.get_hub_result(min, round, message.cr, self.name, message.consumer.name,
                                        final_incentive, final_delay, result_count, Omega, Omega_prime,
                                        message.consumer)
            return M
        else:
            endTime = time.time()
    ...
    def get_result(self, min, round, cr, final_incentive, final_delay, endTime, locked_transfer_endtime, result_count,
        ...
        if len(self.experiment_result.delay) == result_count:
            ...
            if self.contract_table[cr].target.experiment_result.complete == True:
                processing_tx = self.experiment_result.complete_payment + self.contract_table[
                    cr].target.experiment_result.complete_payment
    ...
    def save_result(self, node):
        ...
        for row_index in range(2, len(node.experiment_result.delay) + 2):
            ...
            sheet1.cell(row=row_index, column=13).value = node.experiment_result.protocol_time[complete_round][
                "propose"]
            sheet1.cell(row=row_index, column=14).value = node.experiment_result.protocol_time[complete_round]["select"]
            sheet1.cell(row=row_index, column=15).value = node.experiment_result.protocol_time[complete_round][
                "lockTransfer"]
    ...
    def get_hub_result(self, min, round, cr, hub1, hub2, final_incentive, final_delay, result_count, Omega, Omega_prime,
                       partner):
        utility = contract_bundle(self.address).get_producer_utility(final_incentive, final_delay, Omega)
        if final_incentive == 0:
            self.experiment_result.zero_incentive += 1
    ...
    def save_hub_result(self, node):
        ...
        for row_index in range(2, len(node.experiment_result.contract_delay) + 2):
            ...
            for i in range(len(node.experiment_result.h1_contract_bundle[index]["Incentive"])):
                contract1 += "(" + str(node.experiment_result.h1_contract_bundle[index]["Incentive"][i][
                                           0] / contract_meaningful_incentive_constant) + ", " + \
                             str(node.experiment_result.h1_contract_bundle[index]["Delay"][
                                     i] / contract_meaningful_delay_constant) + "), "

            for i in range(len(node.experiment_result.h2_contract_bundle[index]["Incentive"])):
                contract2 += "(" + str(node.experiment_result.h2_contract_bundle[index]["Incentive"][i][
                                           0] / contract_meaningful_incentive_constant) + ", " + \
                             str(node.experiment_result.h2_contract_bundle[index]["Delay"][
                                     i] / contract_meaningful_delay_constant) + "), "
    ...
    def receive_unlockBP(self, message, round):
        ...
        if self.contract_table[message.cr].target != self:
            ...
            self.update_contract_selection_standard(message.cr, consumer, final_incentive)

    def update_contract_selection_standard(self, cr, partner, final_incentive):
