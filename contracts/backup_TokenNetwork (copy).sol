...
//            uint256(MessageTypeId.BalanceProofUpdate),
        ...
        returns (bytes32, uint256)
    {
        ...
        lockhash = keccak256(abi.encodePacked(locked_amount, expiration, secrethash, selected_incentive, selected_delay, aux_incentive, aux_delay, start_time, target));

        locked_amount = calculateLockedAmount(locked_amount, expiration, secrethash, target, start_time, [selected_incentive, aux_incentive], [selected_delay , aux_delay]);
    ...
    function calculateLockedAmount (
        ...
        if (reveal_time == 0 || expiration <= reveal_time) {
            ...
            for (i=0; i<incentive_bundle.length; i++){
